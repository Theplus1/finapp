...
### 4. API Calls - React Query (MANDATORY)
- **ALWAYS** use React Query hooks: `useMutation` and `useQuery`
- **Use `useMutation`** for CREATE, UPDATE, DELETE operations
- **Use `useQuery`** for READ operations
- **NEVER** call `api.*` directly in components
- See [REACT_QUERY_STANDARD.md](./docs/REACT_QUERY_STANDARD.md) for complete guide
...
**For Mutations (Login, Create, Update, Delete):**
```tsx
...
export default function LoginPage() {
  ...
  return (
    <form onSubmit={handleSubmit}>
      {loginMutation.error && <div>{loginMutation.error.message}</div>}
...
export default function UsersPage() {
  const { data, isLoading, error } = useQuery({
    queryKey: ['users'],
    queryFn: () => api.users.getUsers({ page: 1 }),
  })
...
- Always provide `queryKey` for queries
